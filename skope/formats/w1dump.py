...
# ///
"""
w1dump.py - a standalone script to read and parse w1dump files
...
as a tool:
    ./scripts/w1dump.py dump.w1dump              # full analysis
    ./scripts/w1dump.py dump.w1dump --summary    # summary only
    ./scripts/w1dump.py dump.w1dump --full       # show all memory regions

as a module:
    from w1dump import load_dump, W1Dump
...
def load_dump(path: Union[str, Path], validate: bool = True) -> W1Dump:
    ...
    path = Path(path)
    log.debug_f("loading dump: [%s]", path)
    ...
    try:
        with open(path, "rb") as f:
            data = f.read()
        log.debug_f("read %d bytes from dump file", len(data))
    except IOError as e:
        raise ValueError(f"failed to read dump file: {e}")

    # unpack from msgpack
    log.debug("unpacking msgpack data")
    try:
        dump_dict = msgpack.unpackb(data, raw=False, strict_map_key=False)
        # log.debug(
        #     "unpacked dump dict",
        #     field("keys", list(dump_dict.keys())),
        #     field("size", len(str(dump_dict))),
        # )
    except msgpack.exceptions.ExtraData as e:
        raise ValueError(f"dump file contains extra data, may be corrupted: {e}")
    except Exception as e:
        raise ValueError(f"failed to parse msgpack format: {e}")

    # validate basic structure
    if validate:
        # log.debug("validating dump structure")
    ...
    # parse metadata
    # log.debug("parsing metadata section")
    try:
        metadata = DumpMetadata(**dump_dict["metadata"])
        log.debug(
            "parsed metadata",
            field("process", metadata.process_name),
            field("pid", metadata.pid),
            field("arch", metadata.arch),
    ...
    # parse thread state
    # log.debug("parsing thread state")
    try:
        thread = ThreadState(**dump_dict["thread"])
        log.debug(
            "parsed thread state",
            field("thread_id", thread.thread_id),
            field("gpr_count", len(thread.gpr_values)),
            field("fpr_count", len(thread.fpr_values)),
        )
    except (KeyError, TypeError) as e:
        raise ValueError(f"invalid thread section: {e}")

    # parse memory regions
    # log.debug("parsing memory regions")
    regions = []
    total_data_size = 0
    try:
        for i, r in enumerate(dump_dict["regions"]):
            ...
            if "data" in r and r["data"] is not None:
                r["data"] = bytes(r["data"])
                total_data_size += len(r["data"])
            regions.append(MemoryRegion(**r))
        log.debug(
            "parsed memory regions",
            field("count", len(regions)),
            field("total_data_bytes", total_data_size),
        )
    except (KeyError, TypeError) as e:
    ...
    try:
        ...
        log.debug(
            "parsed modules",
            field("count", len(modules)),
            field("names", [m.name for m in modules[:5]]),  # first 5 module names
        )
    except (KeyError, TypeError) as e:
        raise ValueError(f"invalid module at index {i}: {e}")

    # create and return dump object
    dump = W1Dump(metadata=metadata, thread=thread, regions=regions, modules=modules)
    log.debug("dump loaded successfully")
    return dump
